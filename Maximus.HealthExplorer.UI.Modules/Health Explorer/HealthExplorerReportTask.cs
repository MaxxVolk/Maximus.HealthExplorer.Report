...
namespace Maximus.HealthExplorer.UI.Modules
{
  ...
  public class HealthExplorerReportTask : INotifyPropertyChanged, INotifyInitialized
  {
    ...
    private void RunTask()
    {
      ...
      if (Site.GetService(typeof(IManagementGroupServerSession)) is IManagementGroupServerSession mgSession)
      {
        ...
        if (!ConsoleJobs.RunJob(Site.Component, delegate
        {
          ...
          return;
        (Site.GetService(typeof(IConsoleForm)) as IConsoleForm)?.OpenNewWindow(typeof(HealthExplorerReportForm), new HealthExplorerReportStartupContext(mo));
      }
    }
